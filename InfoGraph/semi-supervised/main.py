...
def train(epoch, use_unsup_loss):
    ...
    if use_unsup_loss:
        for data, data2 in zip(train_loader, unsup_train_loader):
            ...
            optimizer.zero_grad()
            #data.y = data.y.reshape(1244,-1)
            #data.y = torch.sum(data.y, dim=1).flatten()
            data.y = data.y.reshape(model(data).shape)
...
def transforms(data):
        ...
        #edge_attr = None
        edge_attr = torch.cat([data.edge_index[0],data.edge_index[1]], dim=-1)
...
if __name__ == '__main__':
    ...
    dataset = pickle.load(open('/content/drive/MyDrive/SpectrumData/HIVspectra_graph_data_full.pickle', "rb")) 
    for graph in dataset:
            graph.y = torch.sum(graph.y)
    print("====== Dataset Loaded ======")
    print('num_features : {}\n'.format(dataset[1].num_features))
    #print("======= Creating Edge attributes ========")
    #dataset = [transforms(data) for data in dataset]
    #print("====== Finished creating edge_attr ======")
    #print("====== Normalising targets ======")
    target = 0
    # Normalize targets to mean = 0 and std = 1.
    # mean = dataset[1].data.y[:, target].mean().item()
    # std = dataset[1].data.y[:, target].std().item()
    # dataset[1].data.y[:, target] = (dataset[1].data.y[:, target] - mean) / std
    
    # data = dataset[1]
    # mean = data.y.mean(dim=0, keepdim=True)
    # std = data.y.std(dim=0, keepdim=True)
    ...
    train_dataset = dataset[10000:40276]
    ...
    if use_unsup_loss:
        unsup_train_dataset = dataset[30276:]
    ...
    model = Net(dataset[1].num_features, dim, use_unsup_loss, separate_encoder).to(device)
